...
const ogImageUrl = computed(() => {
  if (form.invoice && form.preimage) {
    // Use route params for better OG compatibility
    return `${baseUrl}/api/og/${encodeURIComponent(form.invoice)}/${encodeURIComponent(form.preimage)}.png`
  }
  return ''
})
...
const decodedInvoice = computed(() => {
  // Guard: don't try to decode if invoice is empty
  if (!form.invoice) {
    return null
  }
